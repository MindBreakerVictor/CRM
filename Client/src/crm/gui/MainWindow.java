...
public class MainWindow {
    ...
    public MainWindow(Socket socket) throws IOException {
        this.socket = socket;
    ...
    private void initiateIndividualsTab() {
        ...
        addIndividual.addActionListener(e -> {
            try {
                String customerJSON = JSONUtils.insertCustomer(new Individual(individualFirstName.getText(),
                        individualLastName.getText(), individualDeliveryAddress.getText(), individualContactNumber.getText()));
            } catch (InvalidCustomerException exception) {
                new ErrorWindow("Invalid instanceof customer passed to JSONUtils::insertCustomer at line " +
        ...
        individualsTable.getModel().addTableModelListener(e -> {
            if (e.getType() == TableModelEvent.UPDATE) {
                ...
                JSONUtils.updateCustomer(new Individual(firstName, lastName, id, deliveryAddress, contactNumber));
            }
        });
    }
    ...
    private void initiateCompaniesTab() {
        ...
        companiesTable.getModel().addTableModelListener(e -> {
            if (e.getType() == TableModelEvent.UPDATE) {
                ...
                database.updateCustomer(new Company(name, fiscalCode, bankAccount, hqAddress, id, deliveryAddress, contactNumber));
            }
        });
    }
    ...
    private void initiateInvoicesTab() {
        ...
        searchInvoices.addActionListener(e -> {
            if (searchByUidInvoice.getText().equals("") &&
                searchByCustomerName.getText().equals("") &&
                searchByDate.getText().equals("")) {
                updateInvoiceDropDownList(false);
                return;
            }
